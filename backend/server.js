...
// ----- CROSSWORD SERVER PROXY -----
// Forward all /crossword/* and /leaderboard/crossword requests to the local crossword server
app.use(["/crossword", "/leaderboard/crossword"], (req, res) => {
  const CROSSWORD_SERVER_URL = "http://localhost:4002";
  const targetUrl = `${CROSSWORD_SERVER_URL}${req.originalUrl}`;
  ...
  fetch(targetUrl, options)
    ...
    .catch((err) => {
      ...
      res.status(503).json({ error: "Crossword service unavailable", message: err.message });
    });
});
...
function createDbPool() {
  ...
  if (databaseUrl) {
    return mysql.createPool({
      ...
      // Railway-managed MySQL commonly requires TLS from hosted environments.
  ...
  try {
    ...
    const clientBase =
      ...
      (process.env.NODE_ENV === "production"
        ? "https://wisdomwarfare-rust.vercel.app"
        : "http://localhost:3000");
