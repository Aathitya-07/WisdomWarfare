...
const splitCrosswordClues = (clues = []) => {
  const normalizedClues = Array.isArray(clues) ? clues : [];
  return {
    acrossClues: normalizedClues.filter(
      (clue) => clue.direction === 'across' || clue.direction === 'horizontal'
    ),
    downClues: normalizedClues.filter(
...
const CrosswordGame = ({ user, gameCode, gameName, onLogout }) => {
  ...
  const applyCrosswordGridPayload = (data) => {
    if (!mountedRef.current || !hasPlayableGridPayload(data)) {
      return false;
    }
    ...
    if (data.grid && data.acrossClues && data.downClues && data.cellNumbers) {
      ...
      const { acrossClues, downClues } = splitCrosswordClues(clues);
      setCrosswordData({
        grid: data.grid,
        acrossClues,
        downClues,
  ...
  const syncCrosswordStateFromHttp = async () => {
    if (!gameCode || waitingForFreshStartRef.current) {
      return;
    }

    try {
      const statusRes = await fetch(`${API_BASE}/crossword/game-status/${encodeURIComponent(gameCode)}`);
      if (!statusRes.ok) {
        return;
      }
      ...
      if (!statusData?.started || !statusData?.sessionId) {
        if (!gridLoadedRef.current) {
          setWaitingForTeacher(true);
          setStatusMessage(statusData?.message || 'Waiting for teacher to start the crossword');
          setLoading(false);
        }
        return;
      }

      const durationMs = Number(statusData?.durationMs || 6 * 60 * 1000);
      const remainingTimeMs = Math.max(0, Number(statusData?.remainingTimeMs || durationMs));

      setGameDurationMs(durationMs);
      setTimeLeftMs(remainingTimeMs);
      setGameEndsAt(Date.now() + remainingTimeMs);
      if (statusData?.startedAt) {
        gameStartedAtRef.current = Number(statusData.startedAt);
      }

      sessionIdRef.current = statusData.sessionId;
      ...
      const gridRes = await fetch(`${API_BASE}/crossword/grid/${encodeURIComponent(gameCode)}`);
      if (!gridRes.ok) {
        setWaitingForTeacher(true);
      ...
      if (gridData?.sessionId) {
        sessionIdRef.current = gridData.sessionId;
      }

      const applied = applyCrosswordGridPayload(gridData);
      if (!applied) {
        setWaitingForTeacher(true);
        setStatusMessage('Teacher started the crossword. Loading puzzle...');
        setLoading(false);
      ...
      console.error('HTTP crossword state sync failed:', error);
      if (!gridLoadedRef.current) {
        setLoading(false);
      }
    }
  ...
  useEffect(() => {
    if (!gameCode || waitingForFreshStart) {
      return undefined;
    }
    ...
    const runSync = async () => {
      ...
      await syncCrosswordStateFromHttp();
    };

    runSync();
    const poller = setInterval(runSync, 3000);

    return () => {
      cancelled = true;
      clearInterval(poller);
    };
  }, [gameCode, waitingForFreshStart]);
  ...
  useEffect(() => {
    ...
    newSocket.on('connect', () => {
      ...
      // If the socket room events are delayed/missed, HTTP sync keeps state progressing.
      syncCrosswordStateFromHttp();
      fetchLeaderboard();
    });
    ...
    newSocket.on('crosswordStatus', (data) => {
      ...
      if (started && data?.sessionId) {
        ...
        // Explicitly request the active grid so late joiners or missed broadcasts still receive it.
        newSocket.emit('crosswordRequestGrid', {
          sessionId: data.sessionId,
          game_code: gameCode,
          user_id: user.user_id || user.uid,
        });
    ...
    newSocket.on('crosswordGrid', (data) => {
      ...
      applyCrosswordGridPayload(data);
    });
